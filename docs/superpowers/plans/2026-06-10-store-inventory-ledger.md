...
- [ ] **Step 1: 替换所有 purchase_order_id → source_type/source_id**

改动点（按 location 替换）：

```python
# _confirm_items 中:
"purchase_order_id": order_id  →  "source_type": "purchase", "source_id": order_id
...
# import_confirm 中:
"purchase_order_id": order.id  →  "source_type": "purchase", "source_id": order.id
purchase_order_id=order.id     →  source_type="purchase", source_id=order.id
```

替换 `get_by_purchase_order(order_id)` → `get_by_source_reason("purchase", order_id, "purchase")`，取消时查原始记录用 `get_by_source("purchase", order_id)`。
...
- [ ] **Step 1: 替换所有 retail_order_id → source_type/source_id**

```python
# create_sale 中:
"retail_order_id": retail_order.id  →  "source_type": "retail", "source_id": retail_order.id
retail_order_id=retail_order.id     →  source_type="retail", source_id=retail_order.id

# list_sales 中:
StockMovement.retail_order_id → StockMovement.source_id + StockMovement.source_type == "retail"
Transaction.retail_order_id   → Transaction.source_id + Transaction.source_type == "retail"
...
`get_by_retail_order(order_id)` → `get_by_source_reason("retail", order_id, "retail")`
...
- [ ] **Step 1: 替换所有 return_order_id → source_type/source_id**

- 库存变动: `"return_order_id": order.id` → `"source_type": "return", "source_id": order.id`
- 交易记录: `return_order_id=order_id` → `source_type="return", source_id=order_id`
- 查询: `StockMovement.return_order_id` → `StockMovement.source_id` + `StockMovement.source_type == "return"`
- `Transaction.return_order_id` → 同理

`get_by_return_order(order_id)` → `get_by_source_exclude_reason("return", order_id, "cancel")`
...
### Task 13: 适配 WastageService

**Files:**
- Modify: `backend/app/services/wastage_service.py`

- [ ] **Step 1: 替换所有 wastage_order_id → source_type/source_id**

- 库存变动: `"wastage_order_id": order.id` → `"source_type": "wastage", "source_id": order.id`
- 查询: `StockMovement.wastage_order_id` → `StockMovement.source_id` + `StockMovement.source_type == "wastage"`

`get_by_wastage_order(order_id)` → `get_by_source_exclude_reason("wastage", order_id, "cancel")`

- [ ] **Step 2: 提交**

```bash
git add backend/app/services/wastage_service.py
git commit -m "refactor: WastageService 适配 source 多态"
```
...
- [ ] **Step 1: 替换所有 subscription_order_id → source_type/source_id**

- 库存变动: `"subscription_order_id": order_id` → `"source_type": "subscription", "source_id": order_id`
- 交易记录: `subscription_order_id=order_id` → `source_type="subscription", source_id=order_id`
...
- [ ] **Step 1: 替换 delivery_id → source_type/source_id**

```python
# settle 中:
delivery_id=delivery_id  →  source_type="delivery", source_id=delivery_id
