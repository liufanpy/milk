...
- 毛利未扣减退款额，退款影响单独在退款额列查看
- 成本通过现有 `stock_movements` JOIN `products` 反推，不需要新表或新字段
- 历史数据无需迁移，新口径对旧数据同样适用

## 已知局限

1. **成本依赖实时进价**：成本查询 `JOIN products.default_purchase_price`，若产品进价后续调整，历史日期的毛利数字会随之变化。现阶段不做成本快照，使用时需知晓此特性。
2. **前后端适配**：dashboard 返回字段变动后，前端需同步更新，否则页面可能报错。
