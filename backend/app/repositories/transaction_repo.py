...
class TransactionRepository:
    ...
    def get_amounts_by_distribution(self, distribution_ids: list[int]) -> dict[int, dict]:
        ...
        rows = (
            ...
            .filter(
                Transaction.source_id.in_(distribution_ids),
            )
            .group_by(Transaction.source_id, Transaction.category)
            .all()
        )
        result: dict[int, dict] = {did: {"total_amount": 0.0, "paid_amount": 0.0} for did in distribution_ids}
        for row in rows:
            if row.category in (TransactionCategory.distribution,):
                result[row.source_id]["total_amount"] += row.total
            elif row.category == TransactionCategory.payment:
