...
class ProductService:
    ...
    def export_csv(self) -> str:
        ...
        headers = ["名称", "品牌", "分类", "单位", "条码", "规格", "默认进货价", "零售默认价", "批发默认价", "保质期天"]
        lines = [",".join(headers)]
        for p in products:
            lines.append(",".join([
                p.name, p.brand, p.category, p.unit, p.barcode, p.spec,
                str(p.default_purchase_price),
        ...
        for row in rows:
            ...
            try:
                self.repo.create(
                    ...
                    default_purchase_price=float(data.get("default_purchase_price") or data.get("默认进货价") or 0),
